...
本页用于场景等 source-preserve 素材，或用户明确选择抠图/局部修边时；默认 AI 重绘图标按 [transparent-generation.md](transparent-generation.md) 处理，不能因一次生成失败自动进入本页的抠图、遮罩或旧内区混合路线。出现原图底板色、邻接装饰色、白边或黑边时，先明确当前素材模式。透明 Alpha、平滑轮廓、干净 RGB 是三项独立条件；有半透明像素不代表无杂色。

优先验证内置工具直接输出的真实 RGBA；若只有轻微半透明（例如实体 Alpha 253/255）且换底正常，保留原 Alpha，不进入去污染/二值化返工。仅在有具体杂色证据时执行下面的修复。

## 验收强度

默认按 [acceptance.md](acceptance.md) 的约 8–9 分视觉目标修边，重点是画风一致、目标尺寸下干净且无布局溢出。以下逐像素保护、全底色放大审计和语义合同用于明显缺陷或严格任务，不是每个对象的必经步骤。只有用户明确要求源像素不变时才启用 `sourceRgbIntegrity`；普通局部编辑允许不改变画风的细微色值和纹理变化。
...
白/黑/洋红/青色与实际 UI 底色逐段检查外沿和孔沿，既看部分 Alpha，也看贴着它的实体边沿。半透明带通常在最终 1× 占约 1–2 px；高分辨率工作图按比例理解，不能强制其也只占 1–2 px。透明边距、孔洞透底、内区高光、描边粗细、碎片与原图配准满足当前任务的视觉目标后即可写入；非严格任务不因极端放大下的微小差异反复返工。

严格模式中，不要用 `sourceRgbIntegrity` 要求受污染边带仍逐像素等于源图；该项只约束明确要求保留的可靠不透明内区，边带改变单独记录并目视复核。如果旧 Alpha=255 边缘已污染，先修 Alpha 或局部重建，不能直接禁用源 RGB 验收。原图合成层只能覆盖确认干净的内区：根据实际污染宽度和缩放滤波范围选择保护带，不能默认内缩 1 px 就安全，也不能为了保留源色把旧污染覆盖回来。
